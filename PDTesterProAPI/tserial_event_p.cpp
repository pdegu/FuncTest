...
#define SIG_MODEM_CHECKED  6
	...
	*/
	int  Tserial_event::connect(wchar_t* port_arg, int  rate_arg, int parity_arg,
	...
	void Tserial_event::OnEvent(unsigned long events)
	{
		...
		if ((events & EV_RING) != 0)
		{
			...
			{
				if (manager != 0)
					manager(this, parent, SERIAL_RING);
			}
		}
		...
		if ((events & EV_RLSD) != 0)
		{
			...
			{
				if (manager != 0)
					manager(this, parent, SERIAL_CD_ON);
			}
			else
			{
				if (manager != 0)
					manager(this, parent, SERIAL_CD_OFF);
			}
		}
	}
	/* -------------------------------------------------------------------- */
	...
	*/
	...
	void Tserial_event::run(void)
	{
		...
		if (manager != 0)
			manager(this, parent, SERIAL_CONNECTED);

		GetLastError();               // just to clear any pending error
		//6SetEvent(serial_events[SIG_READ_DONE]);
		if (check_modem)
			...
			else
			{
				...
				case SIG_READ_DONE:
					...
					if (!rx_in_progress)
					{
						...
						//debug
						//wchar_t	str[20];
						//swprintf_s(str, L"Read:%d", max_rx_size);
						//AddLogEntry(str, 4);
				...
				case SIG_READER:
						...
						&result_nbr, FALSE))
					{
						...
						if (manager != 0)
							manager(this, parent, SERIAL_DATA_ARRIVAL);
				...
				case SIG_DATA_TO_TX:
					...
					//debug
					//wchar_t	str[20];
					//swprintf_s(str, L"Write:%d", tx_size);
					//AddLogEntry(str, 4);
				...
				case SIG_WRITER:
						...
						&result_nbr, FALSE))
					{
						...
						if (manager != 0)
							manager(this, parent, SERIAL_DATA_SENT);
					}
					else
					{
						// GetOverlapped didn't succeed !
		...
		if (manager != 0)
			manager(this, parent, SERIAL_DISCONNECTED);
	}
	/* -------------------------------------------------------------------- */
	/* -------------------------------------------------------------------- */
